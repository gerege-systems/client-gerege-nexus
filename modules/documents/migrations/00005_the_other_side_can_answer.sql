...
-- Тенантын ролийг 00001-ийн адил хайна: цөмийн 00079 (v1.14.0)
-- `gerege_nexus_app`-ыг `gerege_nexus_tenant` болгосон тул аль байгааг нь
-- хэрэглэнэ.
-- +goose StatementBegin
DO $rls$
...
BEGIN
    ...
    EXECUTE 'DROP POLICY IF EXISTS counterparty_visibility ON document_eid_sign_sessions';
    EXECUTE 'DROP POLICY IF EXISTS counterparty_visibility ON document_signatures';
    EXECUTE 'DROP POLICY IF EXISTS signatures_are_not_erased ON document_signatures';
    IF app_role IS NULL THEN
        RETURN;
    END IF;

    EXECUTE format($p$
        CREATE POLICY counterparty_visibility ON document_eid_sign_sessions TO %I
    ...
    $p$, app_role);
    EXECUTE format($p$
        CREATE POLICY counterparty_visibility ON document_signatures TO %I
            USING      (counterparty_tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid)
            WITH CHECK (counterparty_tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid)
    $p$, app_role);
    ...
    EXECUTE format($p$
        CREATE POLICY signatures_are_not_erased ON document_signatures
            AS RESTRICTIVE FOR DELETE TO %I USING (false)
    $p$, app_role);
END $rls$;
-- +goose StatementEnd
