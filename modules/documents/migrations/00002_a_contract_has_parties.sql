...
-- Тенантын ролийг нэрлэхийн оронд хайна — 00001-ийн адил: платформын 00029
-- түүнийг `gerege_nexus_app` нэрээр үүсгэсэн, цөмийн 00079 (v1.14.0)
-- `gerege_nexus_tenant` болгосон, аль нэгийг нь бататгасан миграц нөгөө
-- талдаа `role ... does not exist` гэж унана.
-- +goose StatementBegin
DO $rls$
DECLARE
    t TEXT;
...
BEGIN
    ...
    FOREACH t IN ARRAY ARRAY[
        ...
        $p$, t, app_role);
        EXECUTE format($p$
            CREATE POLICY counterparty_visibility ON %I TO %I
    ...
    EXECUTE 'DROP POLICY IF EXISTS parties_see_each_other ON document_parties';
    EXECUTE 'DROP POLICY IF EXISTS parties_are_created_by_the_issuer ON document_parties';
    EXECUTE 'DROP POLICY IF EXISTS signatories_belong_to_their_party ON document_party_signatories';
    EXECUTE 'ALTER TABLE document_party_files ENABLE ROW LEVEL SECURITY';
    EXECUTE 'ALTER TABLE document_party_files FORCE  ROW LEVEL SECURITY';
    EXECUTE 'DROP POLICY IF EXISTS tenant_isolation ON document_party_files';
    EXECUTE 'DROP POLICY IF EXISTS counterparty_visibility ON document_party_files';
    ...
    EXECUTE format($p$
        CREATE POLICY parties_see_each_other ON document_parties FOR SELECT TO %I
    ...
    EXECUTE format($p$
        CREATE POLICY parties_are_created_by_the_issuer ON document_parties
            AS RESTRICTIVE FOR INSERT TO %I
            WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid)
    $p$, app_role);
    ...
    EXECUTE format($p$
        CREATE POLICY tenant_isolation ON document_party_files TO %I
            USING      (tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid)
            WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid)
    $p$, app_role);
    EXECUTE format($p$
        CREATE POLICY counterparty_visibility ON document_party_files TO %I
    ...
    EXECUTE format('GRANT SELECT, INSERT ON document_party_events TO %I', app_role);
END $rls$;
-- +goose StatementEnd
